    ...
    class PostsAdapter extends RecyclerView.Adapter<PostViewHolder> {
        ...
        public void onBindViewHolder(@NonNull PostViewHolder holder, int position) {
            ...
            if (userId.equals(postUserId)) {
                ...
                holder.deleteImageView.setOnClickListener(v -> eliminarPost(postId));
            } else {
                holder.deleteImageView.setVisibility(View.GONE);
            }

            // Configuración del autor y contenido
            if (post.get("authorPhotoUrl") == null) {
                holder.authorPhotoImageView.setImageResource(R.drawable.user);
            } else {
                Glide.with(getContext()).load(post.get("authorPhotoUrl").toString())
                        .circleCrop().into(holder.authorPhotoImageView);
            }
            ...
            if (post.get("mediaUrl") != null) {
                ...
                if ("audio".equals(post.get("mediaType").toString())) {
                    Glide.with(requireView()).load(R.drawable.audio)
                            .centerCrop().into(holder.mediaImageView);
                } else {
                    Glide.with(requireView()).load(post.get("mediaUrl").toString())
                            .centerCrop().into(holder.mediaImageView);
                }
            ...
            // Función Share/Forward
            holder.shareImageView.setOnClickListener(v -> {
                String contenido = post.get("content").toString();
                String autor = post.get("author").toString();
                String mediaUrl = post.get("mediaUrl") != null ?
                        post.get("mediaUrl").toString() : "";

                String textoCompartir = "🌟 Post de " + autor + ":\n\n"
                        + contenido
                        + "\n\n"
                        + (mediaUrl.isEmpty() ? "" : "Media: " + mediaUrl)
                        + "\n\nCompartido desde SocialApp";

                Intent shareIntent = new Intent(Intent.ACTION_SEND);
                shareIntent.setType("text/plain");
                shareIntent.putExtra(Intent.EXTRA_SUBJECT, "Mira este post");
                shareIntent.putExtra(Intent.EXTRA_TEXT, textoCompartir);

                try {
                    startActivity(Intent.createChooser(shareIntent, "Compartir usando..."));
                } catch (Exception e) {
                    Snackbar.make(holder.itemView, "Error al compartir", Snackbar.LENGTH_SHORT).show();
                }
            });
        }
    ...
    void eliminarPost(String postId) {
        ...
        databases.deleteDocument(
                ...
                new CoroutineCallback<>((result, error) -> {
                    mainHandler.post(() -> {
                        if (error != null) {
                            Snackbar.make(requireView(), "Error al eliminar: " +
    ...
    void obtenerPosts() {
        ...
        try {
            databases.listDocuments(
                    ...
                    new CoroutineCallback<>((result, error) -> {
                        if (error != null) {
                            Snackbar.make(requireView(), "Error al obtener posts: " +
